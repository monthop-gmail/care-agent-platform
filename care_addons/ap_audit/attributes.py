...
ทะเบียนนี้ใช้กับ `error/v1` `details` ด้วย — `error/v1` นิยามมันว่า `{"type": "object"}`
คือ open bag รูปเดียวกับ `metadata` เป๊ะ ต่างกันแค่ไม่มีใครพูดถึงมันตอนตกลงเรื่องนี้

ข้อที่กฎนี้ **ไม่ได้** ปิด และต้องพูดให้ตรง: `transition.reason` เป็นฟิลด์ข้อความที่
`event/v1` นิยามเองและไม่มีข้อจำกัดใดในสัญญา · เหตุผลที่คนพิมพ์ตอนหยุดคำสั่งของหมอยังอยู่ที่นั่น
เพราะ ADR-0006 บังคับว่าต้องตอบได้ว่าหยุดเพราะอะไร และไม่มีที่อื่นเก็บ
(`error/v1` `message` ต่างออกไป — มีกฎห้าม PII อยู่แล้วในสัญญา จึงไม่ใช่ของที่รอใคร)
ดู ADR-0011
"""
...
# ── สิ่งที่ใส่ใน `transition.reason` ได้ (ADR-0012) ───────────────────────────
#
# `transition.reason` เป็น `type: string` เปล่า ๆ ในสัญญา ไม่มีอะไรกันไว้เลย
# เราเคยปล่อยให้ค่าที่ route รับมาไหลลงไป **แปดทาง** รวมถึงคำถามที่ผู้ป่วยพิมพ์เอง
# และโน้ตของผู้ดูแลเรื่องการล้ม
#
# 🔒 ข้อนี้ไม่ใช่ความสะอาด — มันคือเงื่อนไขที่ทำให้ erasure มีความหมาย
#    audit ลบไม่ได้ ถ้าประโยคของคนอยู่ในนั้น การลบแถวของโดเมนก็ไม่ได้ลบอะไรจริง
#
# ตัวตรวจบอกได้แค่ว่า *ชื่อไหน* ถูกแทรกเข้าไป บอกไม่ได้ว่าค่าข้างในมาจากใคร
# บรรทัดที่สองจึงเป็นคำตอบของคนว่าทำไมชื่อนั้นไม่ใช่ข้อความเสรี
REASON_INTERPOLATIONS: dict[str, str] = {
    "status": "สถานะของ careplan task / inventory item — ตรวจกับชุดปิดก่อนใช้",
    "state": "สถานะของ safety event — ชุดปิด",
    "kind": "ชนิดของสัญญาณความปลอดภัย — ชุดปิด",
    "role": "บทบาทในบ้าน (patient/caregiver) — ชุดปิด",
    "binding.role": "บทบาทของ LINE binding — ชุดปิด",
    "entry_type": "ชนิดของบันทึกใน journal — ชุดปิด",
    "event": "ชื่อเหตุการณ์จากอุปกรณ์ — ของนอกระบบ แต่เป็น token ไม่ใช่ประโยค",
    "step.awaits_external_event": "ชื่อเหตุการณ์ที่ขั้นตอนรออยู่ — token",
    "step.kind": "ชนิดของขั้นเตรียมตัว — ชุดปิด",
    "source_kind": "ที่มาของงาน — ชุดปิด",
    "source_id": "id ของต้นทาง — ตัวชี้",
    "job.attempts": "จำนวนครั้งที่เตือนไปแล้ว — ตัวเลข",
    "scope.principal.type": "ชนิดของผู้กระทำ (human/agent/service) — ชุดปิด",
}
