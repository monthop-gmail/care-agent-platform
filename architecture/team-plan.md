...
| [agent-platform#25](https://github.com/monthop-gmail/agent-platform/issues/25) | `consent/v1` ตรวจได้แค่ revoked/expired — ใบที่ผูกกับข้อเท็จจริงที่เปลี่ยนได้ (หมอลาออกจากโรงพยาบาล) ยังใช้ได้ต่อ | ✅ รับเข้า `consent/v1` v1.1.0 (ADR-0014) พร้อมกฎ fail-closed ทั้ง 4 ข้อของเรา + ข้อที่ห้า · ปรับรูปเป็น `{kind, params}` — เรา migrate ครบแล้ว |
| [agent-platform#25](https://github.com/monthop-gmail/agent-platform/issues/25) (ต่อเนื่อง) | ไม่มีที่บันทึกว่า "อนุญาตด้วยใบไหน เงื่อนไขผ่านตอนไหน" | ✅ `consent/v1` v1.2.0 `Evaluation` + `event/v1` v1.4.0 field `consent` (ADR-0016) — เราเก็บผลที่แช่แข็งแล้วทุกครั้งที่ตรวจสิทธิ์ รวมถึงครั้งที่ถูกปฏิเสธ |
| [agent-platform#23](https://github.com/monthop-gmail/agent-platform/issues/23) | `event/v1` ไม่มี sequence (ของทีม `devfactory-core` — เราให้ข้อมูลราคาของ contiguous) | ✅ `event/v1` v1.3.0 เพิ่ม `sequence` แบบ **ไม่รับประกันความต่อเนื่อง** ตามที่เราทักว่า rollback สร้างช่องว่างโดยไม่มีใครทำผิด · เราย้ายมาใช้ชื่อกลาง `sequence` แล้ว |
| [devfactory-core#26](https://github.com/monthop-gmail/devfactory-core/issues/26) | ครึ่งหลังของ #23 — trail ที่ถูกตัดท้าย ปิดด้วย `sequence` ไม่ได้ | ✅ RFC-0012 ประกาศ guarantee "ทุก terminal ต้องออกใบปิดท้าย" (semantics 1.2) · เรารับมาใช้: `JOB_SETTLED` ที่ทุกการปิดงาน และ **`JOB_COMPLETED` เหลือเฉพาะตอนส่งมอบสำเร็จจริง** |
