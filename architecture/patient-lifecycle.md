...
ตัดสินแล้วที่ [ADR-0012](../decisions/0012-erasure-cuts-the-bridge-not-the-trail.md)
ตามทางเลือก A ของ `dec-d1302f96`: **erasure ตัดสะพานไปหาตัวตน ไม่ใช่ลบ trail**
— `care_patient.erasure.erase_patient()` ลบแถวของโดเมนทุกตารางที่ถือ `patient_id`
แล้ว trail ยังตอบได้ว่าเกิดอะไรขึ้นเมื่อไร แต่ตอบไม่ได้ว่าของใคร

🔒 **ถอน consent กับ ขอให้ลบ เป็นคนละการกระทำ** และเดินคนละทางโดยเจตนา —
ถ้า implement ทับกัน บันทึกจะบอกว่าเจ้าของข้อมูลถอนความยินยอม ทั้งที่เขาขอให้ลบตัวตน

⏳ ที่ยังไม่มี: ขั้นตอนยืนยันก่อนลบจริง (จึงยังไม่มี HTTP route) และคำถามทางกฎหมายว่า
audit trail เข้าข่ายข้อยกเว้นเรื่องฐานการเก็บหรือไม่ — ยังไม่มีใครถามคนรู้กฎหมาย
